...
namespace InventorySorter
{
    ...
    public class CargoSorterSessionComponent : MySessionComponentBase
    {
        ...
        private readonly HashSet<MyDefinitionId> _allOres = new HashSet<MyDefinitionId>();
        private readonly HashSet<MyDefinitionId> _allIngots = new HashSet<MyDefinitionId>();
        private readonly HashSet<MyDefinitionId> _allComponents = new HashSet<MyDefinitionId>();
        private readonly HashSet<MyDefinitionId> _allAmmo = new HashSet<MyDefinitionId>();
        private readonly HashSet<MyDefinitionId> _allTools = new HashSet<MyDefinitionId>();
        private readonly HashSet<MyDefinitionId> _allBottles = new HashSet<MyDefinitionId>();
        private readonly HashSet<MyDefinitionId> _allConsumables = new HashSet<MyDefinitionId>();
        private readonly HashSet<MyDefinitionId> _allIngredients = new HashSet<MyDefinitionId>();
        ...
        private readonly WcApi _wcApi = new WcApi();
        private readonly HashSet<MyDefinitionId> _weapons = new HashSet<MyDefinitionId>();
        private readonly HashSet<MyDefinitionId> _ignoredAmmoWeapons = new HashSet<MyDefinitionId>();
        private readonly Dictionary<string, MyDefinitionId> _wcAmmoMagazines = new Dictionary<string, MyDefinitionId>();
        ...
        public override void LoadData()
        {
            ...
            foreach (var definition in MyDefinitionManager.Static.GetPhysicalItemDefinitions())
            {
                ...
                if (definition is MyConsumableItemDefinition)
                {
                    ...
                    if (!isInRecipe)
                    {
                        _allConsumables.Add(definition.Id);
                        MakeNormalizedId(definition.Id, "Item");
                    }
                    else
                    {
                        _allIngredients.Add(definition.Id);
                        MakeNormalizedId(definition.Id, "Item");
                    }
                }
                ...
                {
                    _allTools.Add(definition.Id);
                    MakeNormalizedId(definition.Id, "Item");
                }

                if (definition.Id.TypeId == typeof(MyObjectBuilder_PhysicalObject))
                {
                    if (!(_allConsumables.Contains(definition.Id) || _allIngredients.Contains(definition.Id) || _allTools.Contains(definition.Id)))
                    {
                        bool isInRecipe = false;
                        foreach (var consumable in _allConsumables)
                        {
            ...
            foreach (var definition in MyDefinitionManager.Static.GetHandItemDefinitions())
            {
                ...
                if (handPhysicalItem != null && handPhysicalItem.Enabled && handPhysicalItem.Public)
                {
                    ...
                    _allTools.Add(handPhysicalItem.Id);
                }
            }
        ...
        private void OnWcReady()
        {
            ...
            _wcApi.GetAllCoreWeapons(allCoreWeapons);
            ...
            _wcApi.GetAllWeaponMagazines(allWeaponMagazines);
            foreach (var weaponMagazines in allWeaponMagazines)
            {
                ...
                foreach (var magazine in weaponMagazines.Value)
                {
                    ...
                    _wcAmmoMagazines[magazine.Item2.Item3] = magazine.Item2.Item1;
                    shouldIgnore = false;
                    break;
                }
                ...
                if (shouldIgnore)
                {
                    _ignoredAmmoWeapons.Add(weaponMagazines.Key);
                }
            }
        }
        ...
        private void OnMessageEntered(string messageText, ref bool sendToOthers)
        {
            ...
            else if (messageText.StartsWith("/asort", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (_autoSortingController == shipController)
                {
                    MyAPIGateway.Utilities.ShowMessage("Sorter", "Already auto sorting");
                    return;
                }

                _autoSortingController = shipController;
                _autoSortTicksRemaining = 0;
        ...
        private void OnControlledEntityChanged(IMyControllableEntity lastEntity, IMyControllableEntity currentEntity)
        {
            ...
            if (_autoSortingController != null && currentBlock != null && _autoSortingController.CubeGrid.IsSameConstructAs(currentBlock.CubeGrid))
            {
                return;
            }
        ...
        private void DisableAutoSort()
        {
            ...
            _autoSortingController = null;
            MyAPIGateway.Session.LocalHumanPlayer.Controller.ControlledEntityChanged -= OnControlledEntityChanged;
        }

        public override void UpdateBeforeSimulation()
        {
            _autoSortTicksRemaining--;
            if (_autoSortTicksRemaining > 0 || !_jobTask.IsComplete)
            {
                return;
            }
            ...
            BeginConstructSortJob(_autoSortingController.CubeGrid, _autoSortProfile, ResultsDisplayType.None);
        }
        ...
        public void BeginSortJob(IMyCubeGrid rootGrid, string profile, ResultsDisplayType resultsDisplayType)
        {
            ...
            _jobTask = MyAPIGateway.Parallel.Start(SortInventoryAction, SortInventoryCallback, workData);
        }
        ...
        public void BeginQuotaJob(IMyAssembler assembler, ResultsDisplayType resultsDisplayType)
        {
            ...
            _jobTask = MyAPIGateway.Parallel.Start(SetProductionQuotasAction, SetProductionQuotasCallback, workData);
        }
            ...
            foreach (var block in blocks)
            {
                ...
                for (int i = 0; i < block.InventoryCount; i++)
                {
                    ...
                    else if (inventoryInfo.TypeRequests.HasFlag(TypeRequests.ConsumableAmmo))
                    {
                        ...
                        if (_ignoredAmmoWeapons.Contains(inventoryInfo.Block.BlockDefinition))
                        {
                            continue;
                        }
        ...
        private void GenerateInventoryBuckets(List<InventoryInfo> inventories, CargoSorterWorkData workData)
        {
            ...
            foreach (var inventory in inventories)
            {
                ...
                foreach (var definitionId in workData.TypeBuckets.Keys)
                {
                    ...
                    if (inventory.TypeRequests.HasFlag(TypeRequests.Ores) && _allOres.Contains(definitionId) ||
                        inventory.TypeRequests.HasFlag(TypeRequests.Ingots) && _allIngots.Contains(definitionId) ||
                        inventory.TypeRequests.HasFlag(TypeRequests.Components) && _allComponents.Contains(definitionId) ||
                        inventory.TypeRequests.HasFlag(TypeRequests.Ammo) && _allAmmo.Contains(definitionId))
                    {
        ...
        private MyFixedPoint CalculateAmountWanted(InventoryInfo inventoryInfo, MyDefinitionId definitionId, MyFixedPoint currentValue, CargoSorterWorkData workData)
        {
            ...
            if (inventoryInfo.TypeRequests.HasFlag(TypeRequests.Ores) && _allOres.Contains(definitionId))
            {
                return inventoryInfo.ComputeAmountThatFits(definitionId);
            }

            if (inventoryInfo.TypeRequests.HasFlag(TypeRequests.Ingots) && _allIngots.Contains(definitionId))
            {
                return inventoryInfo.ComputeAmountThatFits(definitionId);
            }

            if (inventoryInfo.TypeRequests.HasFlag(TypeRequests.Components) && _allComponents.Contains(definitionId))
            {
                return inventoryInfo.ComputeAmountThatFits(definitionId);
            }

            if (inventoryInfo.TypeRequests.HasFlag(TypeRequests.Ammo) && _allAmmo.Contains(definitionId))
            {
                return inventoryInfo.ComputeAmountThatFits(definitionId);
            }

            if (inventoryInfo.TypeRequests.HasFlag(TypeRequests.Tools) && _allTools.Contains(definitionId))
            {
                return inventoryInfo.ComputeAmountThatFits(definitionId);
            }

            if (inventoryInfo.TypeRequests.HasFlag(TypeRequests.Bottles) && _allBottles.Contains(definitionId))
            {
                return inventoryInfo.ComputeAmountThatFits(definitionId);
            }

            if (inventoryInfo.TypeRequests.HasFlag(TypeRequests.Consumables) && _allConsumables.Contains(definitionId))
            {
                return inventoryInfo.ComputeAmountThatFits(definitionId);
            }

            if (inventoryInfo.TypeRequests.HasFlag(TypeRequests.Ingredients) && _allIngredients.Contains(definitionId))
            {
                return inventoryInfo.ComputeAmountThatFits(definitionId);
            }
        ...
        private string BuildAllSortableItemNamesString()
        {
            ...
            foreach (var item in MakeSortedIdDefs(_allOres))
            {
                sbMap.AppendFormat("{0} is {1}\n", item.Key, item.Value);
                sbInv.AppendFormat("{0}=All\n", item.Key);
            }

            foreach (var item in MakeSortedIdDefs(_allIngots))
            {
                sbMap.AppendFormat("{0} is {1}\n", item.Key, item.Value);
                sbInv.AppendFormat("{0}=All\n", item.Key);
            }

            foreach (var item in MakeSortedIdDefs(_allComponents))
            {
                sbMap.AppendFormat("{0} is {1}\n", item.Key, item.Value);
                sbInv.AppendFormat("{0}=All\n", item.Key);
            }

            foreach (var item in MakeSortedIdDefs(_allAmmo))
            {
                sbMap.AppendFormat("{0} is {1}\n", item.Key, item.Value);
                sbInv.AppendFormat("{0}=All\n", item.Key);
            }

            foreach (var item in MakeSortedIdDefs(_allTools))
            {
                sbMap.AppendFormat("{0} is {1}\n", item.Key, item.Value);
                sbInv.AppendFormat("{0}=All\n", item.Key);
            }

            foreach (var item in MakeSortedIdDefs(_allBottles))
            {
                sbMap.AppendFormat("{0} is {1}\n", item.Key, item.Value);
                sbInv.AppendFormat("{0}=All\n", item.Key);
            }

            foreach (var item in MakeSortedIdDefs(_allIngredients))
            {
                sbMap.AppendFormat("{0} is {1}\n", item.Key, item.Value);
                sbInv.AppendFormat("{0}=All\n", item.Key);
            }

            foreach (var item in MakeSortedIdDefs(_allConsumables))
            {
                sbMap.AppendFormat("{0} is {1}\n", item.Key, item.Value);
                sbInv.AppendFormat("{0}=All\n", item.Key);
            }
